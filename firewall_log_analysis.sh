...
awk '/DENY/ {print $4}' "$log_file" | sort | uniq -c | sort -rn 
...
awk '/DENY/ {print $4}' "$log_file" | sort | uniq -c | awk '$1 > 5 {print $2}' | cat -n
...
unique_denied_ips=$(awk '/DENY/ {print $4}' "$log_file" | sort -u | wc -l)
